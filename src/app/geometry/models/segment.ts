...
export function getBoundingSegments(segment: Segment): [Segment, Segment, Segment, Segment] {
...
export function areSegmentsIntersecting(segmentAB: Segment, segmentCD: Segment): boolean {
    ...
    if (productABAC === 0 && isPointOnSegment(segmentAB, segmentCD[0])) { return true; }
    if (productABAD === 0 && isPointOnSegment(segmentAB, segmentCD[1])) { return true; }
    if (productCDCA === 0 && isPointOnSegment(segmentCD, segmentAB[0])) { return true; }
    if (productCDCB === 0 && isPointOnSegment(segmentCD, segmentAB[1])) { return true; }

    return false;
}

function isPointOnSegment(segment: Segment, point: Point): boolean {
    return Math.min(segment[0].x, segment[1].x) <= point.x && point.x <= Math.max(segment[0].x, segment[1].x)
        && Math.min(segment[0].y, segment[1].y) <= point.y && point.y <= Math.max(segment[0].y, segment[1].y);
}